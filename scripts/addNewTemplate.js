...
/**
 * Replaces hyphens inside a valid domain string with an underscore because
 * hyphens are not valid JS variable names so we cannot use them as
 * autogenerated variables for imports
 * @param {string} domainString
 */
function replaceHyphens(domainString) {
  return domainString.replace("-", "_");
}

function makeSubdirectoryImport(subDir) {
  return `import ${replaceHyphens(subDir)} from "./${subDir}";`;
}

function makeSubdirectoryExports(subDirs) {
  return subDirs.map(subDir => `...${replaceHyphens(subDir)}`).join(", ");
}

export function validateDomainString(domainString) {
  const domainRegexTest = /^([a-z0-9]([a-z0-9-]{0,61}[a-z0-9])?)$/;
  if (domainRegexTest.test(domainString)) {
    return domainString;
  }
  throw new Error(
    `${domainString} was passed in as a subdomain but it is not a valid subdomain`
  );
}

export function generateIntermediateIndexTemplate({ subDirs, currDir }) {
  const subDirectoryImports = subDirs
    .map(validateDomainString)
    .map(makeSubdirectoryImport)
    .join("\n");
