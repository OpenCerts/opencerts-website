...
import {isoDateToLocal, isFireFox, isChrome, isEdge, sassClassNames} from '../common';
import {NUS_TS_BACKIMG} from '../common';
...
const setColWidth = () => {
    let html = 
        <tr>
            <td className={cls("ts-col0 no-padding")}></td>
            <td className={cls("ts-col1 no-padding")}></td>
            <td className={cls("ts-col2 no-padding")}></td>
            <td className={cls("ts-col3 no-padding")}></td>
        </tr>
        ;
    return html;
}
...
const renderTranscriptTermHeader = () => {
    let html =
        <Fragment>
            <td className={cls("ts-title ts-highlight")}><u>MODULE</u></td>
            <td className={cls("ts-title ts-highlight")}>&nbsp;</td>
            <td className={cls("ts-title ts-highlight")}><u>GRADE</u></td>
            <td className={cls("ts-title ts-highlight")}><u>CREDITS</u></td>
        </Fragment>
        ;
    return html;
}
            ...
            <div ref={this.myRef}>
                ...
                <tbody key="pers-info-tbody">
                    ...
                    <tr key="pers-info-tr">
                        ...
                        <td colwidth="12.1%">{isoDateToLocal(jsonData.recipient.dateOfBirth)}</td>
            ...
            <tbody>
                <tr>
                    <td className={cls("footer-text")}>
                        <div id={'footer-' + this.props.page}></div>
                    </td>
                </tr>
            </tbody>
            </table>
...
class TranscriptProgram {
    // main render
    render() {
        let progData = jsonData.additionalData.programData; 
        if (progData)
            progData.forEach((data) => {
                if (data.statusCode !== 'DC') 
                    this.renderProgData(data);
            })
    }

    // render for a program
    renderProgData(data) {
        dataFeeder.push('ts-prog', 
            <td colSpan="4"> 
                <div className={cls("prog-row ts-title")}> 
                    <div className={cls("prog-key prog-col0")}>PROGRAMME:</div> 
                    <div className={cls("prog-col1")}>{data.programName.toUpperCase()}</div> 
                </div>
            </td>
        );
        dataFeeder.push('ts-prog',
            <td colSpan="4"> 
                <div className={cls("prog-row ts-title")}> 
                    ...
                    <div className={cls("prog-col1")}>{data.statusDescription.toUpperCase()}</div>
                </div>
            </td>
        );
    };
...
class TranscriptCreditTransfer {
    ...
    render() {
        ...
        let internal = false;
        let external = false;
        for (let i=0; i<this.termData.creditTransfer.length; i++) {
            let transferData = this.termData.creditTransfer[i];
            if (transferData.sourceType === 'E')  
                external = true;
            else if (transferData.sourceType === 'I') 
                internal = true;
            if (internal && external) break;
        }
        if (this.termIdx === 0) { // only print in 1st term
            if (external) { 
        ...
        } else { // only print for 2nd term onward
            this.renderIntTrfSummary();
        }
        this.renderIntTrfDetail();
        if (this.termIdx !== 0) { // from 2nd term onward
    ...
    renderExtTrfTitle() {
        dataFeeder.push('ts-term-trf-extitle',
    ...
    renderAPC() {
        this.termData.creditTransfer.forEach((transferData) => {
            if (transferData.sourceType === 'E' &&
                transferData.creditsNoGPA > 0) { // APC
                dataFeeder.push('ts-term-trf-apc',
                    <Fragment>
                        <td colSpan="2" className={cls("ts-termrem")}>
                            AWARDED ADVANCED PLACEMENT CREDITS FOR THE ACADEMIC WORK COMPLETED AT {transferData.orgName.toUpperCase()}
                        </td>
                        <td className={cls("ts-grade")}>-</td>
                        <td className={cls("ts-credits")}>{transferData.creditsNoGPA.toFixed(2)}</td>
                    </Fragment>
                ); 
            } 
        })
    ...
    renderIntAPC() {
        this.termData.creditTransfer.forEach((transferData) => {
            if (transferData.sourceType === 'I') {
                let isNUSAPCTest = (transferData.orgId === 'E0000002277');
                let isAPC = (transferData.orgId === 'E0000002430');
                let title;
                if (!isNUSAPCTest && !isAPC) {
                    title = 'CREDITS RECOGNISED ON ADMISSION (NUS MODULES COMPLETED PRIOR TO CURRENT PROGRAMME):';
                } else if (isNUSAPCTest) {
                    title = 'AWARDED ADVANCED PLACEMENT CREDITS FOR PASSING THE PLACEMENT TEST(S) CONDUCTED BY NUS';
                } else if (isAPC) {
                    title = 'AWARDED ADVANCED PLACEMENT CREDITS';
                }
                let grade = (isNUSAPCTest || isAPC)?'-':'';
                let credits = (transferData.creditsNoGPA !== 0)?transferData.creditsNoGPA.toFixed(2):'';
                dataFeeder.push('ts-term-trf-intapc',
                    <Fragment>
                        <td span="2" className={cls("ts-termrem")}>{title}</td>
        ...
        this.termData.creditTransfer.forEach((transferData) => {
            if (transferData.sourceType === 'I') {
                ...
                if (transferData.sourceCareer) {
                    title = 'CREDITS RECOGNISED (COMPLETED MODULES FROM ' + transferData.sourceCareer + ' CAREER)';
                } else {
                    title = 'CREDITS RECOGNISED (COMPLETED MODULES FROM OTHER PROGRAMME)';
                } 
                let credits = (transferData.creditsNoGPA !== 0)?transferData.creditsNoGPA.toFixed(2):'';
                dataFeeder.push('ts-term-trf-inttrf',
                    <Fragment>
                        <td span="2" className={cls("ts-termrem")}>{title}</td>
                        <td>&nbsp;</td>
                        <td className={cls("ts-credits")}>{credits}</td>
            ...
            if (transferData.sourceType === 'I') {
                transferData.details.forEach((detail) => {
                    if (detail.status === 'P' 
                        ...
                        let credits = (detail.credits!==0)?detail.credits.toFixed(2):'-';
                        dataFeeder.push('ts-term-trf-inttrfdtl',
                            <Fragment>
                                <td className={cls("ts-col0 ts-modcode")}>{detail.moduleCode}</td>
                                <td className={cls("ts-col1 ts-modname")}>{detail.moduleName}</td>
    ...
    renderTrfFromExtOrg() {
        this.termData.creditTransfer.forEach((transferData) => {
            if (transferData.sourceType === 'E' 
                ...
                dataFeeder.push('ts-term-trf-fromorg',
                    <Fragment>
                        <td colSpan="2" className={cls("ts-termrem")}>
    ...
    renderTrfEqualNUS() {
        this.termData.creditTransfer.forEach((transferData) => {
            if (transferData.sourceType === 'E' 
                    && transferData.creditsGPA > 0) {
                dataFeeder.push('ts-term-trf-eqnus',
                    <td colSpan="4">CREDITS TRANSFERRED (WITH EQUIVALENT NUS GRADE) FROM {transferData.orgName}:</td>
                ); 
                transferData.details.forEach((detail) => {
                    if (detail.status === 'P' && 
                            (detail.includeInGPA 
                                || (detail.grade === 'S' || detail.grade === 'CS'))) {
                        let credits = (detail.credits!==0)?detail.credits.toFixed(2):'-';
                        dataFeeder.push('ts-term-trf-eqnusdtl',
                            <Fragment>
                                <td className={cls("ts-col0 ts-modcode")}>{detail.moduleCode}</td>
                                <td className={cls("ts-col1 ts-modname")}>{detail.moduleName}</td>
                                <td className={cls("ts-col2 ts-grade")}>{detail.grade}</td>
...
class TranscriptModuleEnroll {
    ...
    // main render
    render() {
        let moduleCode=this.data.moduleCode;
        if ((this.data.gradingBasis === 'NCP' && this.data.includeInGPA) 
                || (this.data.remarks)) 
            moduleCode = '*' + moduleCode;
        let credits = (this.data.credits===0)?'-':this.data.credits.toFixed(2);
        dataFeeder.push('ts-term-enl-mod',
            <Fragment>
...
class TranscriptEnrollment {
    ...
    renderEnrollTitle() {
        dataFeeder.push('ts-term-enl-title',
            <td colSpan="4" className={cls("ts-termrem")}>
    ...
    renderSupplementaryTitle() { 
        dataFeeder.push('ts-term-enl-suptitle',
            <td colSpan="4" className={cls("ts-termrem")}>
...
class TranscriptSummary {
    ...
    render() {
        this.termData.summary.forEach((data) => {
            ...
            // GPA
            if (data.specialGPA) 
                this.renderSpecialGPA(data);
            else 
                this.renderGPA(data);
            // term honours
            if (data.awards)
                this.renderTermHonours(data);
        });
    }

    // render degree
    renderTermDegree(sumData) {
        dataFeeder.push('ts-term-deg',
    ...
    renderSpecialGPA(sumData) {
        sumData.specialGPA.forEach((data, i) => {
            let name = (data.type==='FCAP')?'*':'' + data.name;
            dataFeeder.push('ts-term-sgpa',
                <td colSpan="4" className={cls("ts-termrem ts-highlight")}>
    ...
    renderTermHonours(sumData) {
        sumData.awards.forEach((data, i) => {
            dataFeeder.push('ts-term-awd',
                <td colSpan="4" className={cls("ts-termrem ts-highlight")}>
...
class TranscriptTermRemarks {
    ...
    renderRemarksTitle() {
        dataFeeder.push('ts-term-rem-title',
            <td colSpan="4" className={cls("ts-termrem ts-highlight")}>
                <p/>
                <u>REMARKS:</u>
            </td>
    ...
    renderEnrollRemarks() {
        ...
        this.termData.modules.forEach((data) => {
            if (data.includeInGPA && data.gradingBasis === 'NCP') {
                this.cache.push('ts-term-rem-ncp',
                    <td colSpan="4" className={cls("ts-termrem")}>
                        *{data.moduleCode} - Module was excluded from computation of the final Cumulative Average Points/Marks.
                    </td>
                );
            }
        });
        ...
        this.termData.modules.forEach((data) => { 
            if (data.remarks) {
                this.cache.push('ts-term-rem-oth',
                    <td colSpan="4" className={cls("ts-termrem")}>
        ...
        if (this.termData.creditTransfer && this.termData.formOfStudy !== 'NOC') {
            this.termData.creditTransfer.forEach((transferData) => {
                if (transferData.sourceType === 'E') {
                    ...
                    let isAPC = (transferData.orgId === 'E0000002430');
                    if (!isNUSAPCTest && !isAPC) {
                        this.cache.push('ts-term-rem-trf',
                            <td colSpan="4" className={cls("ts-termrem")}>
                                Please refer to the transcript of {transferData.orgName} for details of modules taken and grades/credits obtained.
                            </td>
                        );
                    }
                }
    ...
    renderFinalGPARemarks() {
        ...
        let hasFinalGPA = this.termData.summary.some((data) => {
            let found = false;
            if (data.specialGPA) {
                found = data.specialGPA.some((sdata) => {
                    if (sdata.type === 'FGPA')
                        return true;
                });
                if (found)
        ...
        this.cache.push('ts-term-rem-fgpa',
            <td colSpan="4" className={cls("ts-termrem")}>
                The Final Cumulative Average Point takes into account student's academic performance at the Partner University.
            </td>
        );
    }
    ...
    renderTranscriptTexts() {
        ...
        this.cache.push('ts-term-rem-txt',
            <td colSpan="4" className={cls("ts-termrem")}>{text}</td>
        );
    }
...
class TranscriptLeave {
    ...
    renderLeave(data) {
        ...
        if (data.to)
            text += ' TO ' + isoDateToLocal(data.to);
        dataFeeder.push('ts-loa',
            <td colSpan="4" className={cls("ts-title ts-highlight")}>
                <hr/>
                {text}
            </td>
...
class TranscriptDegree {
    ...
    renderSubplans(data) {
        ...
        data.plans.forEach((planData, planIdx) => {
            if (planData.subplans) 
                planData.subplans.forEach((subplData, subplIdx) => {
                    descr = subplData.typeName + ': ' + subplData.transcriptDescr;
                    dataFeeder.push('ts-deg-spln',
                        <td colSpan="4" className={cls("ts-title ts-highlight")}>
                            &nbsp;&nbsp;&nbsp;&nbsp;{descr.toUpperCase()}
                        </td>
                    );
    ...
    renderSpecializations(data) {
        ...
        let descr;
        data.specializations.forEach((splData, splIdx) => { 
            descr = splData.typeName + ': ' + splData.transcriptDescr; 
            dataFeeder.push('ts-deg-spcl',
                <td colSpan="4" className={cls("ts-title ts-highlight")}>
                    &nbsp;&nbsp;&nbsp;&nbsp;{descr.toUpperCase()}
                </td>
            ); 
    ...
    // render degree
    renderDegree(data, degIdx) {
        let degTitle = data.degreeTitle.toUpperCase(); 
        if (data.honours) {
            if (data.isYNC) 
                degTitle += ', ' + data.honours;
            else
                degTitle += ' with ' + data.honours;
        }
        dataFeeder.push('ts-deg-title',
            <td colSpan="4" className={cls("ts-title ts-highlight")}>
    ...
    render() {
        ...
        this.msData.forEach((data) => {
            if (data.milestoneTitle) {
                let descr = data.milestoneTitle + ": " + data.thesisTitle;
                dataFeeder.push('ts-ms',
                    <td colSpan="4" className={cls("ts-title ts-highlight")}>
                        &nbsp;&nbsp;&nbsp;&nbsp;{descr.toUpperCase()}
                    </td>
                );
...
class TranscriptSpclProg {
    ...
    render() {
        if (!this.degreeData) return '';
        this.degreeData.forEach((data, degIdx) => {
            if (data.plans) 
                data.plans.forEach((planData, planIdx) => {
                    if (planData.specialProgram) {
                        let descr = planData.transcriptDescr;
                        if (planData.planDescr) 
                            descr += ' ' + planData.planDescr;
                        dataFeeder.push('ts-splprg',
                            <td colSpan="4" className={cls("ts-title ts-highlight")}>
                                &nbsp;&nbsp;&nbsp;&nbsp;{descr.toUpperCase()}
                            </td>
                        );
...
class TranscriptAward {
    ...
    renderAwardDetails() {
        this.awardData.forEach((data) => {
            dataFeeder.push('ts-awd-l1',
                <td colSpan="4" style={{paddingTop: '0', paddingBottom: '0'}}>
                    <div className={cls("awd-col0")}></div>
...
class TranscriptData {
    ...
    renderConferDate() {
        let degreeData = jsonData.additionalData.degreeData;
        if (!degreeData) return '';
        let date = isoDateToLocal(degreeData[0].dateConferred);
        dataFeeder.push('ts-confdt',
            <td colSpan="4" className={cls("ts-title ts-highlight")}>
    ...
    renderDegreeRemarks() {
        let remarksData = jsonData.additionalData.remarks;
        if (!remarksData) return '';
        let text = '';
        remarksData.forEach((line) => {
            text += line.trim() + ' ';
        });
        dataFeeder.push('ts-degrem',
            <td colSpan="4" className={cls("ts-title ts-highlight")}>
                {text}
            </td>
        );
...
class TranscriptPage extends Component {
    ...
    renderColumn(pageIdx, colIdx) {
        ...
        for (let i=0; i<this.props.rowsPerCol; i++) {
            html.push(
    ...
    render() {
        ...
        let idx = this.props.pageIdx;
        let html = 
            <div className={cls("a4-landscape ts-background")} style={backgroundImg}>
                <header key="ts-header"><TranscriptHeader /></header>
                <article key="ts-content">
                    <div className={cls("table ts-text")} id={"canvas-"+idx}
                        style={{height: CONTENT_HEIGHT}}>
                        <table width="100%">
                        <tbody>
                            <tr>
                                <td id={"col-"+idx+'-0'} width="49.5%" valign="top" className={cls("no-padding")}>
                                    {this.renderColumn(idx, 0)}
                                </td>
                                <td width="1%"/>
                                <td id={"col-"+idx+'-1'} width="49.5%" valign="top" className={cls("no-padding")}>
    		    ...
    		    <footer key="ts-footer"><TranscriptFooter page={idx}/></footer>
            </div> 
            ;
        return html;
    }
...
class Transcript extends Component {
    constructor(props) {
        super(props);
        this.dataIdx = 0;
        this.maxPages = (this.props.maxPages)?parseInt(this.props.maxPages):MAX_PAGES;
        this.maxRows = (this.props.maxRows)?parseInt(this.props.maxRows):MAX_ROWS_PER_COL;
        this.page = 0;
        this.col = 0;
        this.row = 0;
    ...
    cleanup() {
        // clean up pages
        for (let i=this.maxPages-1; i>this.page; i--) 
        ...
        for (let i=0; i<=this.page; i++) {
            for (let j=0; j<2; j++) 
                for (let k=this.maxRows; k>=0; k--) {
                    let node = this.rowEl(i, j, k);
                    if (node)
                        if (!node.filled) 
                            node.remove();
                        else {
    ...
    componentDidMount() {
            ...
            if (gState.getState()) {    
                ...
                if (this.col === 0) {
                    ...
                    if (this.page >= this.maxPages) {
                        ...
                        alert('Max pages reached. Some data cannot be rendered.')
                        return;
                    } 
                    this.col = 0;
                    this.row = 0;
                ...
                if (this.dataIdx > dataFeeder.termDataRange.end) {
                    dataRow = <TranscriptDataRow data={dataFeeder.data(this.dataIdx)} parent={node} />;
                } else {
                    dataRow = <TranscriptDataRow data={renderTranscriptTermHeader()} parent={node} />;
                    this.dataIdx--; // re-render the row
                }
                ...
                node = this.rowEl(this.page, this.col, this.row);
                if (this.dataIdx === dataFeeder.termDataRange.start 
        ...
        for (let i=0; i<this.maxPages; i++) {
            contentBottom.push(this.pageContentBottom(i));
        }
        // print first row
    ...
    renderFooters() {
        let totalPages = this.page+1;
        for (let i=0; i<totalPages; i++) {
            let footer = this.footerEl(i);
            if (footer)
                ReactDOM.render(
    ...
    renderPage(idx) {
        let html =
            <TranscriptPage pageIdx={idx} rowsPerCol={this.maxRows}/>
            ;
        return html;
    }

    // get content bottom for a page
    pageContentBottom(idx) {
        let rect = this.canvasEl(idx).getBoundingClientRect();
...
class TranscriptDataRow extends Component {
    ...
    componentDidMount() {
        ...
        let watermark = rect.top + rect.height + window.scrollY;
        let pageIdx = parseInt(this.parent.id.split('-')[1]);
        let cap = contentBottom[pageIdx];
        if (watermark > cap) {
            gState.dispatch({type: "CHANGE_COLUMN"});
...
// ========================================
// set up global state
const onPrintRow = function(nextCol=false, action) {
    switch(action.type) {
    case "NO_CHANGE":
        return false;
    case "CHANGE_COLUMN":
