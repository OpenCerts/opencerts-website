...
export const formatDate = dateString => {
  ...
  var yearStr = toWords(year);
  
  // change - begin with UpperCase, place commas with 'and'
  yearStr = yearStr.replace(',',' and'); 
  var yearReplacedStr = yearStr.substring(0, 1).toUpperCase().concat(yearStr.substring(1));
  return (
    <span>
      {tz(date, TIMEZONE).format("MMM")} <br /> {yearReplacedStr}
    </span>
  );
};
