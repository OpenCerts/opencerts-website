...
export const renderRemarks = certificate => {
  ...
  const remarksRows = remarksArray.map((r, i) => (
    <tr key={i}>
      <div className="mb-2">
...
export const renderAwards = certificate => {
  ...
  const awardRows = awardsArray.map((r, i) => (
    <tr style={{ width: "40%" }} key={i}>
      <div className="mb-2">
  ...
  return awardsArray ? (
    <div>
      ...
      <div className="row m-0 mb-2">
        <div>
          <table className="ml-5" style={tableStyle}>
            {awardRows}
          </table>
        </div>
      </div>
